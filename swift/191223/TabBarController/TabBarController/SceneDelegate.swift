...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func scene(_ scene: UIScene, willConnectTo session: UISceneSession, options connectionOptions: UIScene.ConnectionOptions) {
        ...
        let naviController2 = UINavigationController(rootViewController: secondVC)
        secondVC.title = "SecondVC"

        
        //->퍼스트VC를 네비컨트롤러에 넣어줌, firstVC에 네비게이션컨트롤러
        naviController.tabBarItem = UITabBarItem(title: "First", image: UIImage(systemName: "person.circle"), tag: 0)
        naviController2.tabBarItem = UITabBarItem(title: "Second", image: UIImage(systemName: "folder.fill"), tag: 1)
        thirdVC.tabBarItem = UITabBarItem(title: "Third", image: UIImage(systemName: "paperplane"), tag: 2)
        
        let tabBarController = UITabBarController()
        tabBarController.viewControllers = [naviController, naviController2, thirdVC]
